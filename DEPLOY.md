...
| `AI` | Workers AI: photo suggestions (Qwen3 27B), pantry-drafted dishes (Qwen3 27B), meal planning (Llama 3.3 70B) and plate illustrations (FLUX.1 schnell). Every model answer is re-validated before it reaches D1 or the browser. |
| `PROVIDER_LIMITER` | Per-user rate limit for provider endpoints. |
| `SPOONACULAR_API_KEY` | Optional. Enables Explore's "Search online" and pantry-based online search; results are cached into `recipe_catalog` so later browsing needs no quota. Explore works fully without it. |
...
pages, upserts about 800 recipes with photos into `recipe_catalog`, and repeats
weekly, or on the next tick after a failed or partial run. It uses TheMealDB's
public development key `1`; the provider requires supporter access for a public
app-store release, so revisit that before native distribution. See
[TheMealDB's API access terms](https://www.themealdb.com/api.php). Check a
deployment with:

```bash
npx wrangler d1 execute pantry-pal-staging --remote --env staging --command "SELECT name,last_status,last_finished_at,updated FROM catalog_jobs"
```

Any signed-in client (a guest session is enough) can read the same rows, minus the
lease token, at `GET /api/catalog/jobs`: `status` is `ok`, `partial`, `failed` or
`running`, with `startedAt`, `finishedAt`, `requested`, `updated`, `missing`,
`leased` and `error` (the first failure's reason, such as
`Error: mealdb_fetch_failed:403`, kept from migration 0010 and cleared by a clean
run). A job left `running` and `leased` well past its ten-minute lease means the
invocation was cut off before it could record a result. Outbound Worker fetches
use `redirect: 'manual'` and treat a 3xx answer as a failure: the Workers runtime
rejects `redirect: 'error'` with a TypeError before sending anything, which is
what kept every scheduled fetch failing until 2026-09-23.

Recipe photos uploaded or generated by a household are stored as bounded private
blobs in D1 (`household_images`, migration 0008, at most 150 per household, 900 KB
each after the browser downsizes them) and served only to current members. Move
them to a private R2 bucket if volume grows; the route shape stays the same.

The optional Spoonacular key adds online search to Explore:

```bash
npx wrangler secret put SPOONACULAR_API_KEY --env staging
```

Without it the "Search online" button reports that online search is not set up
