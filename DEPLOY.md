...
`running`, with `startedAt`, `finishedAt`, `requested`, `updated`, `missing` and
`leased`. A job left `running` and `leased` well past its ten-minute lease means
the invocation was cut off before it could record a result.
