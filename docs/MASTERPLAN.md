...
| P6 | Web cutover complete; estate integration open | Canonical domain routes to Worker/D1; live guest and security/asset checks pass; **email delivery verified by the owner on 2026-09-21** (verification email arrived, reset link worked); the first production cron observation remains to be read from `catalog_jobs`. **Still open:** hub card on `ainadara.com`, `pantry` DNS row and uptime probe in `ainadara-infra` — see the review follow-ups |
