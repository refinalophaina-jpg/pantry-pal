...
| Mirror verified on staging (2026-09-23 06:17 UTC) | Verified by fetching; production awaits the owner's dispatch | PR #20 merged as `f0aa4dd`; staging run 35822768501 green (build `b6f89feabcea3571`). Staging's next tick mirrored TheMealDB in 8 s: `themealdb` ok, requested 790, updated 790, missing 0, no error. The recipe index now lists 796 dishes (790 mirrored, every one with a photo, plus the 6 curated) across 40 cuisines, and a mirrored row loads by slug with its ingredients, steps and image. Production still runs the pre-fix Worker and shows 6 dishes until the deploy workflow is dispatched with `environment=production`; its first hourly tick after that should fill the index the same way |
