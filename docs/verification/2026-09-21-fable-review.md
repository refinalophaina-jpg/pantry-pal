...
| R-3 | **Browser journeys not in CI.** `tests/app.spec.ts` existed but only ran locally. | medium | `ci.yml` `browser` job: builds, starts `wrangler dev` with a throwaway secret, runs Playwright Chromium; `continue-on-error` for two weeks (remove after 2026-10-05), report uploaded on failure. |
