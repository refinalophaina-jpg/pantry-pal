...
function PrepContent() {
  ...
  return <div>
    ...
    <Card className="mb-6 space-y-3">
      <h2 className="font-medium">Use the fridge first</h2>
    ...
    <div className="grid xl:grid-cols-2 gap-5">{prepSessions.map(session => <Card key={session.id} className="space-y-4">
      <h2 className="text-lg font-medium">{session.title}</h2><p className="text-sm text-[var(--text-muted)]">{session.shared}</p>
      <div className="grid grid-cols-2 gap-3">{session.recipes.map((id,i) => {const recipe=prepRecipes.find(r=>r.id===id)!;return <button key={id} className="text-left space-y-2" onClick={()=>setOpen(prepBatch(recipe,people))}><FoodVisual name={recipe.name}/><span className="text-xs text-[var(--text-muted)]">{i===0?'Lunch':'Dinner'} · days 1–3</span><span className="block font-medium text-sm">{recipe.name}</span></button>;})}</div>
      <details><summary className="cursor-pointer min-h-11 flex items-center font-medium text-sm">View prep order and shopping list</summary><ol className="list-decimal pl-5 space-y-2 text-sm">{session.order.map(step=><li key={step}>{step}</li>)}</ol>
