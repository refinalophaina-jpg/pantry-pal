import { text, number, nullable, oneOf, unit, bool, date, url, imageRef, id, stringArray, ingredients, type Validator, type SqlValue } from './domain-validation';
...
export const collections: Record<string, Collection> = {
  ...
  'saved-recipes': { table:'saved_recipes',fields:{name:text(),description:nullable(text(12000,true)),cuisine:nullable(text(100)),minutes:nullable(number(0,10080,true)),difficulty:nullable(oneOf(['easy','medium','hard'])),servings:nullable(number(1,1000,true)),equipment:stringArray(),ingredients,steps:stringArray(100,8000),tags:stringArray(),external_id:nullable(text(200)),image_url:nullable(imageRef),area:nullable(text()),source:nullable(text(2048)),video:nullable(url),calories:nullable(number()),protein_g:nullable(number()),carbs_g:nullable(number()),fat_g:nullable(number())},defaults:{equipment:'[]',ingredients:'[]',steps:'[]',tags:'[]'},required:['name'],order:'created_at DESC, id' },
